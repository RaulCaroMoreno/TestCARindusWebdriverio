...
When(
	'"Non_Registered" tries to create the account with empty data',
	async () => {
...
When(
	'"Non_Registered" tries to create the account with correct data',
	async () => {
...
When(
	/^"Non_Registered" tries to create the account with incorrect name (.*)$/,
	async (name) => {
...
When(
	/^"Non_Registered" tries to create the account with incorrect surname (.*)$/,
	async (surname) => {
...
When(
	/^"Non_Registered" tries to create the account with incorrect email (.*)$/,
	async (email) => {
...
When(
	/^"Non_Registered" tries to create the account with password (.*)$/,
	async (password) => {
...
Then(/green tick is (.*) for (.*)$/, async (checkVisibility, check) => {
	expect(await registrationPage.tickMessageIsShown(check)).toBe(
